...
|------|------|--------|--------|-------------|----------------|----------|
| Full | _(default)_ | 6 | 14 | Fable | Mixed (Opus + Sonnet + Fable) | Complex features, architectural decisions |
| Lite | `--lite` | 4 | 6 | Fable | Opus (Sonnet validation) | Standard features, moderate complexity |
| Quick | `--quick` | 2 | 4 | Fable | Opus | Simple features, time-sensitive changes |
...
0. **Stage 0 — Codebase Reconnaissance**: 3 Fable sub-agents explore the actual codebase from different angles (architecture, relevant code, conventions)
...
0. **Stage 0 — Codebase Reconnaissance**: 3 Fable sub-agents (same recon as full)
...
Which model runs each stage is defined by a **panel** (a single table in `bin/acorn`) rather than hardcoded per stage. Defaults: recon → Fable (fast, cheap codebase scanning), synthesis/evaluation/final-spec → Opus (the reasoning-heavy steps), drafters and red-team spread across Opus/Sonnet/Fable so competing plans come from genuinely different models.
...
acorn create <repo> <issue-number> [--no-auto] [--lite | --quick]
    ...
    --lite    Use lite 4-stage pipeline (6 agents, Fable recon)
    --quick   Use quick 2-stage pipeline (4 agents, Fable recon)
