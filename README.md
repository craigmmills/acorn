...
Which model runs each stage is defined by a **panel** (a single table in `bin/acorn`) rather than hardcoded per stage. Defaults: recon → Sonnet (1M context, capable, mid-cost — recon is bulk codebase scanning, so it doesn't warrant a premium model), synthesis/evaluation/final-spec → Opus (the reasoning-heavy steps), drafters and red-team spread across Opus/Sonnet/Fable so competing plans come from genuinely different models. Claude Fable 5 is Anthropic's *most capable* (and most expensive) model, so it is placed on the highest-value reasoning voices (one drafter, one red-teamer), not on cheap bulk work like recon.
